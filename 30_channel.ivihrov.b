...
my %conf = do $set{conf};
#&$debwarn( Dumper \%conf );
my %CHANN_GROUP = do "/usr/local/rle/var/share3/TIKETS/1886/bin/channel-group.conf" or die "Possible, error in /usr/local/rle/var/share3/TIKETS/1886/bin/channel-group.conf ?. Why i cant do it?";
...
# print join( $,, $_, $metka_val||"" );
 ...
 if ($metka_val){
	my  $chgroup = $CHANN_GROUP{ lc $metka_val }{ CHG }||"noGr";
 	print join( $,, $Sid,$Sz,$uid,$second,$geo,$usertraits,$ref,$metka_val."^".$chgroup );
 }
 else{
 	print join( $,, $Sid,$Sz,$uid,$second,$geo,$usertraits,$ref,"noChannel^noGr" );
 }
